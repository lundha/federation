	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		case "Contract":
			...
			case "findContractByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findContractByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindContractByID(ctx, id0)
				if err != nil {
		...
		case "Supplier":
			...
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "Supplier": %w`, err)
			}
			switch resolverName {
			...
			case "findSupplierByID":
				id0, err := ec.unmarshalNID2string(ctx, rep["id"])
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findSupplierByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindSupplierByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "Supplier": %w`, err)
				}
...
func entityResolverNameForContract(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		...
		return "findContractByID", nil
	}
	return "", fmt.Errorf("%w for Contract", ErrTypeNotFound)
}

func entityResolverNameForSupplier(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
			m   map[string]interface{}
			val interface{}
	...
	return "", fmt.Errorf("%w for Supplier", ErrTypeNotFound)
}
